...
Though the STUN servers are often sufficient, sometimes they still can't find a valid NAT mapping between two peers. See branch `werift` for an example using free-to-use TURN servers to relay all communication between peers when the STUN servers fail.
